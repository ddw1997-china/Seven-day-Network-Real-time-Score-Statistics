...
class ExcelRankerApp:
    def __init__(self, root):
        ...
        self.data_start_row = 1  # 数据开始行（排名计算的起始行）
    ...
    def create_widgets(self):
        ...
        # 数据起始行设置（关键修复：这是排名数据的开始行）
        ttk.Label(main_frame, text="数据起始行:").grid(row=1, column=0, sticky=tk.W, pady=8)
        self.data_start_row_var = tk.StringVar(value="2")  # 默认从第2行开始（跳过标题）
        data_start_entry = ttk.Entry(main_frame, textvariable=self.data_start_row_var, width=10)
        data_start_entry.grid(row=1, column=1, sticky=tk.W, padx=(10, 0))
        ttk.Label(main_frame, text="(排名数据开始的Excel行号，通常跳过标题行)").grid(row=1, column=2, sticky=tk.W,
                                                                                    padx=(10, 0))
        ...
        # 数据预览区域
        ttk.Label(main_frame, text="数据预览:", font=("Arial", 10, "bold")).grid(row=10, column=0, sticky=tk.W,
                                                                                 pady=(15, 8))

        # 创建Treeview用于显示数据预览
        preview_frame = ttk.Frame(main_frame)
        preview_frame.grid(row=11, column=0, columnspan=3, sticky=(tk.W, tk.E, tk.N, tk.S), pady=(0, 10))

        # 创建滚动条
        tree_scroll = ttk.Scrollbar(preview_frame)
        tree_scroll.pack(side=tk.RIGHT, fill=tk.Y)

        self.tree = ttk.Treeview(preview_frame, yscrollcommand=tree_scroll.set, height=8)
        tree_scroll.config(command=self.tree.yview)
        self.tree.pack(side=tk.LEFT, fill=tk.BOTH, expand=True)

        # 按钮
        button_frame = ttk.Frame(main_frame)
        button_frame.grid(row=12, column=0, columnspan=3, pady=(20, 15))
        ...
        self.progress.grid(row=13, column=0, columnspan=3, pady=15)
    ...
    def update_data_preview(self):
        ...
        start_display_row = max(0, self.data_start_row - 1)  # 转换为0-based索引

        for i in range(display_rows):
            if start_display_row + i < len(self.df):
                row_data = self.df.iloc[start_display_row + i]
                values = [str(val) for val in row_data.values]
                excel_row_num = self.data_start_row + i
                self.tree.insert("", "end", text=f"Excel第{excel_row_num}行", values=values)
    ...
    def select_file(self):
        ...
        try:
            ...
            # 获取数据起始行
            self.data_start_row = int(self.data_start_row_var.get())
            if self.data_start_row < 1:
                raise ValueError("数据起始行必须大于0")

            # 提取数据区域（从数据起始行开始）
            if self.data_start_row <= len(self.original_df):
                self.df = self.original_df.iloc[self.data_start_row - 1:].reset_index(drop=True)
            else:
                self.df = pd.DataFrame()  # 空DataFrame
            ...
            self.status_var.set(
                f"读取成功: 共{len(self.original_df)}行，数据区域{len(self.df)}行 (从第{self.data_start_row}行开始)")
        ...
        try:
            ...
            if not pd.api.types.is_numeric_dtype(df[data_col]):
                ...
                if na_count > 0:
                    ...
                    excel_rows = [self.data_start_row + idx for idx in na_indices]

                    # 从原始数据获取真实值
                    invalid_details = []
                    for i, (df_idx, excel_row) in enumerate(zip(na_indices[:5], excel_rows[:5])):
                        original_row_idx = df_idx + (self.data_start_row - 1)
                        if original_row_idx < len(self.original_df):
                            original_value = str(self.original_df.iloc[original_row_idx][data_col])
    ...
    def save_results(self):
        ...
        try:
            # 构建完整结果：保留原始标题行 + 添加排名结果
            if self.data_start_row > 1:
                # 保留标题行
                result_df = self.original_df.iloc[:self.data_start_row - 1].copy()

                # 添加排名结果到数据区域
                data_with_rank = self.original_df.iloc[self.data_start_row - 1:].copy()

                # 添加排名列
                for col in self.df.columns:
                    if col not in data_with_rank.columns:
                        data_with_rank[col] = self.df[col]

                # 合并结果
                final_df = pd.concat([result_df, data_with_rank], ignore_index=True)
            else:
                # 如果没有标题行，直接使用带排名的数据
                final_df = self.df
