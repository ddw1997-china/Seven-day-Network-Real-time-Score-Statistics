...
class ContentWidget(QWidget):
    ...
    def handle_logo_clicked(self):
        ...
        if not username or not password:
            ...
            return
        login_stream = LoginSignal()
        login_stream.update_message.connect(self.add_output_message)
        login_stream.login_success.connect(self.login_success)
    ...
    def login_success(self,form_data):
    ...
    def login_check(self, result):
        if result['success'] == False:
            ...
            return
        """添加输出信息到文本框（自动换行）"""
        self.add_output_message(result['message'])

        QMessageBox.information(self, '成功', '登录成功')
        self.logo_tab_btn.setEnabled(False)
    ...
    def create_qitian_table(self):
        ...
        row, col = 0, 0
        for i in range(1, 12):
            className_ = f"8{i if i > 9 else '0' + str(i)}"
            checkbox = QCheckBox(className_,)  # 创建带文本的CheckBox
            ...
            if col >= 4:  # 每行4列
                col = 0
                row += 1
        self.classbox.setLayout(gridLayout)
        # 科目下拉列表
