...
class QiTianApp(QObject):
    ...
    def __init__(self,phone_str:str,pass_str:str,_subject:str,_classlist:list,_progressBar:QProgressBar,_seachbtn:QPushButton):
        ...
        # 解码URL编码字符串
        phone = unquote(phone_str)
        # 输入的URL编码字符串
        # pass_str = "Nuwn7GFxRlM%2BPmOuT8NAkA%3D%3D"
        # 解码URL编码字符串
        password = unquote(pass_str)
        # 用户登录，获取token
    ...
    def getUserInfo(self):
        ...
        response = requests.request("POST", url, json=payload, headers=headers, verify=False)
        info = response.text
        print('==================3.1-getUserInfo:schoolName、subject===============================')
    ...
    @retry(exceptions=requests.exceptions.RequestException, tries=5, delay=1, max_delay=10,backoff=2)
...
class DataToExcel():
    ...
    def saveToExcel(self,_data, _class, _names, _codes, _writer):
        ...
        __data = dict(sorted(_data.items(), key=lambda item: int(re.search(r"\d{1,2}", item[0]).group())))
    ...
    def run(self,qtApp:QiTianApp,classList):
        ...
        # 3. 组合文件路径
        file_path = os.path.join(save_dir, f"{self.examName}.xlsx")
        # if self.is_excel_file_locked(file_path):
        ...
        if os.path.exists(file_path):
            ...
            for attempt in range(max_retries):
                ...
                try:
                    ...
                    # 创建新写入器
                    with pd.ExcelWriter(file_path, engine='xlsxwriter') as _writer:
                        # 重新写入所有原有工作表
                        for name, data in existing_sheets.items():
                            data.to_excel(_writer, sheet_name=name, index=False)
                        for score_value in self.datas:
                            self.saveToExcel(score_value["_data"],score_value["_class"],score_value["_names"],score_value["_codes"],_writer)
                    break  # 成功则退出循环
                except IOError:
                    if attempt < max_retries - 1:
                        ...
                        self.creat_dialog_window(qtApp.progressBar,
                                                 "dialog3", "错误",
                                                 f"{self.examName}.xlsx关闭失败，数据未保存！",
                                                 QMessageBox.StandardButton.Ok,
                                                 QMessageBox.Icon.Warning)
                        return
        else:
            try:
                with pd.ExcelWriter(file_path, engine='xlsxwriter') as _writer:
                    for score_value in self.datas:
                        self.saveToExcel(score_value["_data"], score_value["_class"], score_value["_names"],
                                     score_value["_codes"], _writer)
            except IOError:
                print("新文件写入失败")
        # messagebox.showinfo("","数据已保存完毕")
        # QMessageBox.information(None, '通知', '数据处理完毕！')
        qtApp.updateProgress()
        try:
            self.creat_dialog_window(qtApp.progressBar,
                                     "dialog2", "通知",
